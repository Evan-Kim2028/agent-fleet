...
def register(ctx: HermesPluginContext) -> None:
    ...
    ctx.register_tool(
        ...
        toolset="coding_fleet",
        schema=schemas.CODING_FLEET_PR_REVIEW,
        handler=tools.coding_fleet_pr_review,
        requires_env=["CURSOR_API_KEY"],
    )
    ctx.register_tool(
        name="coding_fleet_pr_loop",
        toolset="coding_fleet",
        schema=schemas.CODING_FLEET_PR_LOOP,
        handler=tools.coding_fleet_pr_loop,
        requires_env=["CURSOR_API_KEY"],
    )
    ctx.register_tool(
        name="coding_fleet_scope",
        toolset="coding_fleet",
        schema=schemas.CODING_FLEET_SCOPE,
