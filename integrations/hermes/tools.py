...
def coding_fleet_scout(args: dict[str, object], **kwargs: object) -> str:
    del kwargs
    try:
        load_fleet_config, _, _ = _ensure_agent_fleet()
    except RuntimeError as exc:
    ...
    depth = str(args.get("depth") or "light").lower()
    ...
    try:
        result = run_scout(
            ...
            github_repo=_optional_str(args.get("github_repo")),
            issue_limit=_int_arg(args.get("issue_limit"), default=20),
            product_context=_optional_str(args.get("product_context")) or "",
            depth=depth,
        )
    except Exception as exc:
        return json.dumps({"error": str(exc)})
