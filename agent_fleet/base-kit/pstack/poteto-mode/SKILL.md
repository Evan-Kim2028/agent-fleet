...
**Defaults for every `Task` call.** `run_in_background: true`, agent mode (readonly strips MCP), file pointers not inlined context, and explicit model (`composer-2.5` for code, `claude-opus-4-7-thinking-xhigh` for prose and judgment).
