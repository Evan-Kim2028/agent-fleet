...
| Tool-iteration cap | `OPENROUTER_MAX_TOOL_ITERATIONS` — integer bound on the tool-use loop (default `200`) |
| Repeated-call window | `OPENROUTER_STALL_WINDOW` — identical tool/argument calls per window before a stall trigger (default `6`) |
| Inactivity window | `OPENROUTER_STALL_MAX_ITERS` — iterations without file mutations or new distinct commands after the first mutation-capable attempt (default `40`) |
| Context budget | `OPENROUTER_CONTEXT_BUDGET_TOKENS` — estimated prompt-token budget; compaction starts above 70% (default `180000`) |
...
| Run fails with `exit=stalled` (exit 3) | The session repeated one identical tool call (`OPENROUTER_STALL_WINDOW` times) or made no mutations/new commands for `OPENROUTER_STALL_MAX_ITERS` iterations after one nudge; fix the prompt or raise the env windows |
| Run fails with `exit=contract_unmet` (exit 2) | The final answer never satisfied the caller's `CompletionContract` (e.g. missing PR URL) within `max_nudges` follow-ups; the last reply is kept in stdout |
...
- **Completion contract:** `session.send(..., completion_contract=CompletionContract(required_patterns=(...), or_blocker_pattern=..., max_nudges=2))` validates the final text against required regexes (e.g. a PR URL) or an explicit blocker statement. An empty or non-matching final message gets one corrective user message per nudge ("Your final message did not satisfy the completion contract: ..."), and after `max_nudges` the run ends with exit code 2 (`exit=contract_unmet`, stderr `completion contract unmet`, last text kept in stdout). An empty final message is nudged even without a contract (`exit=empty_reply` on the same exit code).
- **Stall detection:** the session tracks a rolling window of the last `OPENROUTER_STALL_WINDOW` (default 6) tool calls; the same (tool, args) call repeating across the whole window triggers a stall, as do `OPENROUTER_STALL_MAX_ITERS` (default 40) iterations without a file mutation or a new distinct `run_command` after the first write/edit/command attempt. The first trigger injects one nudge ("You appear to be looping/reading without progress; commit to a change or state the blocker"); a second trigger ends the session with exit code 3 (`exit=stalled`). Read-only/audit personas that keep issuing distinct reads are never aborted.
- **Context budgeting:** once the estimated prompt tokens (last call's `input_tokens` scaled by the history's char/token ratio) exceed 70% of `OPENROUTER_CONTEXT_BUDGET_TOKENS` (default 180000), the session first elides tool-result bodies older than the last 8 (reusing `_HISTORY_ELIDED_STUB`), then — if still over budget — makes one side call asking the model for a ≤400-word progress summary and replaces the history with system + first user turn + that summary + the last 4 turns (tool groups kept intact; never compacted below the first user message). Compaction is logged; if the side call fails it is skipped and the session continues. The per-iteration INFO line includes `input_tokens=` from the last call so an orchestrator can watch context growth.
