...
- Exhausted retries fail the phase with exit code 1; stderr names the failure classification (`rate_limit` / `quota` / `transient` / `timeout` / `error`). Cognition's `Reached overall message rate limit` and `"cognition.ai/errorKind": "unavailable"` are retryable (not a hard `error`).
