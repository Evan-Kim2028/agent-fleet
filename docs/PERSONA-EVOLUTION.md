...
```
~/.agent-fleet/
  runs/<run-id>.jsonl           # per-dispatch timeline
...
skill_slots_review: [pstack/unslop]
...
- `~/.agent-fleet/runs/<run-id>.jsonl` (when `run_id` set)
...
- **Review** — reviewer loadout + **unslop** (`review_skill_slots`)
...
| `agent_fleet/phases.py` | review unslop skills |
