"""Regression tests for capacity-aware orchestration fanout (v0.10.0 run 3)."""
...
from agent_fleet.admission import AdmissionController, ResourceTier, Token
...
class _AdmissionDispatcher:
    """Stand-in whose ``_execute_task`` honors a real admission gate."""

    config: FleetConfig
    _admission: AdmissionController = field(init=False)
    _admission_lock: threading.Lock = field(default_factory=threading.Lock)
...
def _hold_parent_token(dispatcher: _AdmissionDispatcher) -> Token:
    with dispatcher._admission_lock:
        token = dispatcher._admission.try_admit("agent")
...
def _release_parent_token(dispatcher: _AdmissionDispatcher, token: Token) -> None:
    with dispatcher._admission_lock:
        dispatcher._admission.release(token)
...
def test_effective_capacity_defaults() -> None:
    d = _AdmissionDispatcher(config=_config(max_parallel=3))
    assert effective_capacity(d, fallback=99, reserved=0) == 3
    assert effective_capacity(d, fallback=99, reserved=1) == 2


def test_effective_capacity_ram_bound() -> None:
    d = _AdmissionDispatcher(config=_config(max_parallel=10, ram_budget_gb=24))
    assert effective_capacity(d, fallback=99, reserved=0) == 6
    assert effective_capacity(d, fallback=99, reserved=1) == 5
...
def test_nested_dag_wide_rank_completes_without_denials() -> None:
...
def _negative_control_unbounded_once() -> tuple[int, int]:
    """Unbounded pool with parent token held reproduces admission denials."""
    dispatcher = _AdmissionDispatcher(config=_config(max_parallel=3))
    parent_tasks = [
        FleetTask(goal=f"burst-{i}", persona="coder", pipeline="simple", title=f"burst-{i}")
        for i in range(8)
    ]
    token = _hold_parent_token(dispatcher)
    ...
    try:
        ...
        _release_parent_token(dispatcher, token)
    return _count_denied(results), sum(1 for r in results if r.status == "completed")


def test_negative_control_unbounded_pool_denies_with_parent_token() -> None:
    for _ in range(_FLAKES):
        denied, completed = _negative_control_unbounded_once()
        assert denied >= 1, (denied, completed)
        assert completed < 8, (denied, completed)
